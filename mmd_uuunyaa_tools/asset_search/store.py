...
import requests
from mmd_uuunyaa_tools.utilities import strict_hash
...
class ContentStore(StoreABC):
    def __init__(
        self,
        cache_dir: str,
        temporary_dir: str,
        cache_limit_size: int = 1024*1024*1024,
        max_workers: int = 10,
        contents_load: bool = True,
        contents_save_interval_secs: float = 5.0
    ):
        print(f'ContentStore.__init__: cache_dir={cache_dir}, temporary_dir={temporary_dir}')
        self.cache_dir: str = cache_dir
        self.cache_limit_size: int = cache_limit_size
    ...
    def _load_contents(self):
        contents_json_path = os.path.join(self.cache_dir, 'contents.json')
        ...
        with self._lock:
            ...
            self._contents = OrderedDict({
                key: Content(
                    ...
                    filepath=os.path.join(self.cache_dir, value['filepath']),
                    type=value['type'],
                    length=value['length']
    ...
    def _to_content_filepath(self, content_id: str) -> str:
        return os.path.join(self.cache_dir, content_id)
    ...
    def _fetch(self, task: Task):
        ...
        try:
            ...
            with self._lock:
                ...
                content.state = Content.State.STORED
    ...
    def try_get_content(self, url: URL) -> Union[Content, None]:
        ...
        with self._lock:
            ...
            excess_cache_size = max(0, self._contents_size - self.cache_limit_size)
